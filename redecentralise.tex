...
\maketitle
\begin{Introduction}
The intent of the original decentralised design of the Web was to make it easy for individuals to share and access information by simply setting up a web server and using a browser.  Yet, instead of individuals running their own servers, today, a handful of large social and data platform providers serve and host a vast majority of end-user data.  In exchange for this convenience, these platforms have assumed a place of strategic importance in peoples' lives; not only establishing an inextricable dependence on their services, but also the ability to arbitrarily administer targeted advertising, behavioural manipulation, and nearly constant surveillance.

In response, many efforts have sought to `re-decentralise' the web, that is allow end-users to rely less on such brokers, and retain greater control over their data.  In this paper, we critically interrogate what this means; is this a call to extend the model of a Web client to provide end-users with information storage and serving capabilities?  We posit that, architecturally, with increasingly advanced capabilities of web browsers, the Web is ready for increased functionality centred at the end user for fostering greater responsiblity and control.  

Yet, how to deliver this vision of democratisation remains unclear.  In order for any such approaches to be successful, they must be simple, open and, essentially minimal.  

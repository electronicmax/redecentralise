...
\maketitle
\begin{abstract}
The intent of the original decentralised design of the Web was to make it easy for individuals to share and access information by simply setting up a web server and using a browser.  Yet, instead of individuals running their own servers, today, large, centralised information brokers on the Web serve and host a vast majority of end-user data, in exchange for nearly constant surveillance and exposure to advertising.  In response, many efforts have sought to `re-decentalise' the web, that is allow inexperienced end-users to rely less on such brokers, and retain greater control over their data.  In this paper, we critically interrogate what this means; is this a call to extend the model of a Web client to provide end-users with information storage and serving capabilities?  We posit that, architecturally, with increasingly advanced capabilities of web browsers, the Web is ready for increased user responsibility and control. We first analyse 75 ``indie web'' projects that have this goal, deriving four categories of systems. Next, based on the capabilities such systems provide, we provide an analysis of the needs in greatest demand, which, beyond information storage and hosting, include universal addressibility in the face of dynamic network connectivity, private, secure and anonymous communications, and simple programmability.  We then derive a set of design requirements for extending Web clients, and provide the example of a decentralised microblogging environment that maintains the Web's principles of minimal protocol commitment,  in order to achieve maximum heterogeneous interoperability and support for future change.
\end{abstract}
